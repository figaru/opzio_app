...
KNN.prototype.data = getterSetter([], function(arrayOfArrays) {
...
KNN.prototype.clusters = function() {
    var data = this.data();
    var pointsAndCentroids = kmeans(data, {k: this.k(), iterations: this.iterations() });
    ...
    return centroids.map(function(centroid) {
        ...
        return point.label() == centroid.label()
            ...
            return {
              //Add other fields you may want to return
              id: point.id,
...
KNN.prototype.k = getterSetter(undefined, function(value) { return ((value % 1 == 0) & (value > 0)) });

KNN.prototype.iterations = getterSetter(Math.pow(10, 3), function(value) { return ((value % 1 == 0) & (value > 0)) });
...
function kmeans(data, config) {
  ...
  var points = data.map(function(vector) {
    return new Point([vector[0], vector[1]], vector[2].id)
  });
