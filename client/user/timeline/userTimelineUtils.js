...
drawUserTimeline = function(el){
	console.log('drawUserTimeline')
	var container = el[0];
	...
	var options = {
		...
		zoomMin: 1000 * 60 * 15,// * 7,
		zoomMax: 1000 * 60 * 60 * 24,// * 31 * 12,
		min: new Date(Session.get('dateRange').startDate),
	    max: new Date(Session.get('dateRange').endDate),
    	start: new Date(Session.get('dateRange').startDate),
        end: new Date(Session.get('dateRange').endDate),    
	};
...
prepareUserActivityData = function(data){
	console.log('prepareUserActivityData')
